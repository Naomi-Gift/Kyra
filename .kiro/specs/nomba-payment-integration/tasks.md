...
- [ ] 1. Webhook hardening — signature header + idempotency
  - [ ] 1.1 Set `NOMBA_WEBHOOK_SECRET=NombaHackathon2026` in `.env.local`
  - [ ] 1.2 Fix webhook route: change header lookup from `x-nomba-signature` → `nomba-signature` (already done)
  - [ ] 1.3 Add `processedWebhookRefs: Set<string>` to `store.ts`
  - [ ] 1.4 Update `markVirtualAccountPaid`: check set before writing, add ref to set after
  - [ ] 1.5 Update `updatePayoutStatus`: check set before writing, add ref to set after
  - [ ] 1.6 Update webhook route to pass ref into store helpers for deduplication

- [ ] 2. Bank list + payment method API
  - [ ] 2.1 Create `src/lib/banks.ts` with 25 Nigerian banks (codes + names)
  - [ ] 2.2 Create `GET /api/banks` route returning the bank list
  - [ ] 2.3 Create `POST /api/account/payment-method` route calling `store.upsertPaymentMethod`
  - [ ] 2.4 Create `GET /api/account/payment-method` route returning the current payment method

- [ ] 3. New store helpers
  - [ ] 3.1 Add `listAllVirtualAccounts()` to `store.ts`
  - [ ] 3.2 Add `listAllPayoutRecords()` to `store.ts`

- [ ] 4. Status polling endpoint
  - [ ] 4.1 Create `GET /api/payments/status` returning all virtual accounts, payout records, and `lastUpdated`

- [ ] 5. Reconciliation endpoint
  - [ ] 5.1 Create `GET /api/payments/reconciliation` grouping collections + payouts by group + round
  - [ ] 5.2 Include `allPaid` (all VAs for round have `paidAt`), `potTotal` in NGN per cycle

- [ ] 6. Payout retry logic
  - [ ] 6.1 Update `POST /api/payments/payout`: if existing record status is `failed`, reset to `pending` and re-call Nomba Transfer with the same reference
  - [ ] 6.2 Add `resetPayoutForRetry(reference)` helper to store

- [ ] 7. `usePaymentStatus` hook
  - [ ] 7.1 Create `src/hooks/usePaymentStatus.ts`
  - [ ] 7.2 Poll `GET /api/payments/status` every 4s via `setTimeout`
  - [ ] 7.3 Return `{ virtualAccounts, payoutRecords, lastUpdated, loading }`
  - [ ] 7.4 Clean up timeout on unmount
